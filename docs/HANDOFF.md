...
- AI：DeepSeek（两阶段：分类 + 深度解析）；**政治理论**使用 Responses API（`/v1/responses`）内置 `web_search` 联网搜索（时政以检索结果为准并标注来源，失败自动回退普通调用）；其余模块默认全部使用 `deepseek-chat`（`deepseek-reasoner` 已默认停用——耗时数倍且长思考可能截断输出，如需启用可在 `server.js` 的 `callDeepSeek` 第 5 参传入）
